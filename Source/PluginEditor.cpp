    ...
    : AudioProcessorEditor(&p), processor(p)
{
    ...
    configureSlider(tuneSlider, "%"); configureSlider(airSlider, "%"); configureSlider(spaceSlider, "%"); configureSlider(warmthSlider, "%"); configureSlider(outputSlider, " dB");
    ...
    for (auto* s : { &tuneSlider, &airSlider, &spaceSlider, &warmthSlider, &outputSlider, &hpSlider, &lowMidSlider, &presenceSlider, &compSlider, &deEssSlider, &delaySlider, &ratioSlider, &attackSlider, &releaseSlider, &parallelSlider, &ambientSlider, &delayTimeSlider }) addAndMakeVisible(*s);
    tuneA = std::make_unique<SliderAttachment>(processor.parameters, "tuneAmount", tuneSlider); airA = std::make_unique<SliderAttachment>(processor.parameters, "air", airSlider); spaceA = std::make_unique<SliderAttachment>(processor.parameters, "plate", spaceSlider); warmthA = std::make_unique<SliderAttachment>(processor.parameters, "warmth", warmthSlider); outputA = std::make_unique<SliderAttachment>(processor.parameters, "outputGain", outputSlider);
...
void AfroVocalPresetsAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    labels(inputArea.withTop(inputArea.getY() + 42).withHeight(16), { "HPF", "TUNE", "KEY" }); labels(dynArea.withTop(dynArea.getY() + 42).withHeight(16), { "THRESH", "RATIO", "ATTACK" }); labels(toneArea.withTop(toneArea.getY() + 42).withHeight(16), { "LOW MID", "PRESENCE", "AIR" }); labels(fxArea.withTop(fxArea.getY() + 42).withHeight(16), { "DE-ESS", "PLATE", "DELAY" });
...
void AfroVocalPresetsAudioProcessorEditor::resized()
{
    ...
    three(hpSlider, tuneSlider, outputSlider, input.withTop(top)); three(compSlider, ratioSlider, attackSlider, dyn.withTop(top)); three(lowMidSlider, presenceSlider, airSlider, tone.withTop(top)); three(deEssSlider, spaceSlider, delaySlider, fx.withTop(top));
